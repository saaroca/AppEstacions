...
export class MapComponent {
  ...
  onClick(data) {
    ...
    //< SCRIPT >
    //function miFuncion() {
    //  var url = "/Historic?idEstacio=param-id";
    //  url = url.replace("param-id", data.sourceTarget.feature.idEstacio);
    ...
    let panelHtml = '<html>' + '</br>' + `<h4> CURRENT CONDITIONS </h4>` + '</br>' + `<p> Outside Temperature ${geoJson.outTemp}ºC </p>` +
      ...
      '<button type = "button" id="btn" onclick ="miFuncion()"> Comprovar històric </button>' +
      `<SCRIPT>function miFuncion(){ var url ="/Historic?idEstacio=param-id"; url = url.replace("param-id", data.sourceTarget,feature,idEstacio); window.location.href = url; }</SCRIPT>` + '</html>'
